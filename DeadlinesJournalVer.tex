...
The following lemma establishes that we can think of $\EstimateError(\tau,\varepsilon)$ as a polynomial $\{\varepsilon(v)\}_{v\in \tau}$.

\begin{lemma}\label{lemma:poly}
Given a task tree $\tau$, if we consider $\EstimateError(\tau,\varepsilon)$ as a function of the variables $\{\varepsilon(v)\}_{v\in Seq}$ where $seq$ is the set of Sequence nodes in $\tau$, we have a polynomial of maximal degree $|Seq|$.  
\end{lemma}
\begin{proof}
