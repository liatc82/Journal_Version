...
We also assume initially that the random variables are discrete and have finite support (i.e. the number of values
for which the probability is non-zero is finite).
As the resource of interest is assumed to be completion time, we consider each leaf node, $v$, as a random variable that has
a completion-time distribution $P_v$ and a cumulative 
distribution function (CDF) $F_v$. More generally, in this paper, we use the term $P_X$ to denote the (discrete) probability distribution of a (discrete) random variable $X$, i.e., the list of the probabilities of the outcomes, also known as the probability mass function. We use the term $F_X$ to denote the cumulative distribution function, i.e., $F_X(x)$ is the probability that $X$ will take a value less than or equal to $x$.

For clarity, we denote all random variables by symbols of the form $X_{name}$ where in the subscript we put the name of the variable. Similarly we denote nodes in the task trees by symbols of the form $v_{name}$ where the subscript contains the name of the node. The symbol $\tau_{name}$ where the subscript contains a name of a node denotes the corresponding sub-tree (i.e., the node is the root of the sub-tree). We use the symbol $X'_{name}$ to denote an approximation of the random variable $X_{name}$.
...
independent discrete random variables $X_i$. The duration of a sequence \hl{node} $S$ is a random
\hl{variable}:
\[
X = \sum_{c\in child(S)} X_c
\]
...
\[
X = \max_{c\in child(S)} X_c
\]
\hl{Let} $X_r$ be a random variable representing the duration distribution of the root,
and denote the cumulative distribution function (CDF) of a 
random variable $X$ by $F_X$. Then
the probability that the plan meets the deadline $T$ is $F_{X_r}(T)$.
Thus we need to compute the CDF, which is NP-hard~\cite{mohring2001scheduling}. We show how
to deterministically approximate the CDF of the root with additive error at most $\varepsilon $
...
For every deadline $T$ we can easily return the probability for $\tau$ to satisfy $T$. If $T=8$ the probability is  $F_{\tau}(8)=25/1024$ . 
\end{example}
...
\]
Our notion of approximation uses the Kolmogorov distance. \hl{Let} $0\leq\epsilon<1$. %$1 > \epsilon \geq 0$.
\hl{should be $\varepsilon$ or $\epsilon$???}If the following equation holds:
\[
...
which \hl{consists of} $d=(p', x)$ pairs, where $x\in \support(X)$
and $p'$ is the probability $P(X{=}x)$. In the pair $d=(p', x)$, we denote the value $x$
by $val(d)$, and the probability $p'$ by $prob(d)$.
For example, let $X$ be a random variable distributed as: $[0.1:1, 0.1:2, 0.8:4]$, and $X'$
...
computed as in Algorithm~\ref{alg:Trim}. That is, our approximation for a sequence operator (for $n\geq 2$) is \hl{given by}:
\[
...
\hl{Given a task tree $\tau$ and a accuracy requirement $0<\varepsilon<1$, we generate a distribution for a random variable $X_{\tau}' $
...
\noindent So $\sum_{c\in child_v} e_c \leq   \sum_{c\in child_v} \frac{|\tau_{c}|\varepsilon}{|\tau_v|}  \leq \varepsilon  - \varepsilon/ |\tau_v|  $. Then, by Lemma~\ref{appPalTheorem}, using $K=|\tau_v|$ and $n=n_v$, we get that $\Parallel(\{X_c\}_{c\in child_v})\succeq_\varepsilon  X_\tau$ as required. 
\end{proof}
...
\begin{proof}
	By induction over the depth of $\tau$ denoted by $d$. Base: $ d=1$, a single primitive node $v_p$. Primitive nodes do not carry any error, and by procedure \EstimateError line 3, \hl{$P_{v_p}=0$}, 
	therefore  $\CalcNetwork(\tau_{v_p},\varepsilon) \succeq_0 X_{\tau_{v_p}}$. 
	Induction hypothesis: Assume the lemma is true for a task tree with depth $ <d $. Step: we refer to the children nodes of the root node $\ch(v_{root})= \lbrace c_1, c_2,...,c_{|\ch(v_{root})|}\rbrace$ 
	...
	by Theorem~\ref{appSeqTheorem}, sequence node produce an error which is the sum of all children node errors and an additional $\varepsilon(v_{root})$ caused by the trim operator $$\varepsilon_0={\varepsilon(v_{root})+\sum_{c \in \ch(v_{root})}{\EstimateError(c,\varepsilon)}}$$ equivalent to \EstimateError line 7. By the induction hypothesis, each of the children sub trees satisfy the lemma and we get that $\CalcNetwork(\tau_{v_{root}}, \varepsilon) \succeq_{\varepsilon_0} X_\tau$ as required.
	
	\hl{If $v_{root}$ is a parallel node then,}
	by Lemma~\ref{tightPalTheorem}, $$\varepsilon_0=1-\prod_{c \in \ch(v_{root})}{(1-\EstimateError(c))}$$ as in line 11 of \EstimateError. By the induction hypothesis, each of the children sub trees satisfy the lemma and we get that $\CalcNetwork(\tau_{v_{root}}, \varepsilon) \succeq_{\varepsilon_0} X_\tau$ as required.
\end{proof}
...
$$\EstimateError(\tau,\varepsilon)=\sum_{c\in \ch(v_{root})}\EstimateError(c,\varepsilon) +  \varepsilon.$$ 
If $v_{root}$, is a parallel node  $$\EstimateError(\tau,\varepsilon)=1-\prod_{c\in \ch(v_{root})} (1-\EstimateError(c,\varepsilon)).$$ 
