...
Primitive action nodes contain distributions over their resource consumption. Although other resources can be handled with our approach, we will assume henceforth, in order to be more concrete, that the only resource of interest is time.
...
will terminate in time less than $T$?}
...
$$
where $X_c$ is a random variable representing the duration distribution of child node $c$. \hl{We denote by $\ch(v)$ the set of children of some node $v$.}
...
\hl{If the input $D_X$ to $\Trim$ is sorted in increasing order of $x$ (we denote this operator by $\Trim_U(X, \varepsilon)$)
...
lower bound of $X$. From now on, in order to simplify, we will use the notation $\Trim$ instead of $\Trim_U$ but the meaning remains the same.}
...
%\TODO write some intro regarding the algos
...
\begin{algorithm}
 ...
 Let $v$ be the root of $\tau$\;
  ...
  \If{$v$  is a $\operatorname{Sequence}$ node}{
	 ...
	 \Return \Sequence(\hl{$\{X_c\}_{c \in \ch(v)}$, $\varepsilon(v)$})	
...
such that $\CalcNetwork(\tau, \varepsilon)$ is always $\succeq_{\varepsilon_0}$ than $X_{\tau}$. 
...
\begin{lemma} \label{TPar}
	...
	CDF (either all are Discrete, all are Normal, or all are Piecewise polynomial) and let $T \in \mathbb{R}$. Then, deciding \hl{$F_{\max\{Y_1,\dots,Y_n\}}(T)$} %$P(\max\{Y_1,\dots,Y_n\} \leq T) $ 
...
After running our approximation algorithm we also \hl{ran $\Trim_L$ which uses an inverted version of the} $\Trim$ operator, providing a {\em lower} bound of the CDF, as well as the upper bound
...
\begin{table*}[tbh!]
	{\footnotesize
		\begin{tabular}{|l|l|l|| p{1.1cm}|p{1.1cm}|p{1.4cm}||p{0.6cm}|p{0.6cm}|p{0.6cm}|p{0.6cm}|p{0.6cm}|}
			\hline
...
\begin{table*}[tbh!]
	{\footnotesize
		\begin{tabular}{|l|l|l|| p{1.1cm}|p{1.1cm}|p{1.4cm}||p{1.1cm}|p{1.1cm}|p{1.1cm}|}
			\hline
...
%Todo: Add optrim?
\hl{Numerous issues remain unresolved, briefly discussed below. Although the $\Trim$ operator provides nice and efficient approximation, improvements are possible. For instance, not perform trimming (or even stop a trimming operation) if the current support 
