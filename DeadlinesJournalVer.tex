...
\SetKwFunction{CalcNetwork}{GenNetwork}
\SetKwFunction{Solve}{Solve}
\SetKwFunction{NetworkTight}{NetworkTight}
...
% TODO: Liat will expand this.
Although other resources can be handled with our approach (\hl{e.g., memory}: tasks running in parallel use the sum of the memory space of each of the tasks; if they run in sequence, only the maximum thereof is needed), we will assume henceforth, in order to be more concrete, that the only resource of interest is time.
...
\hl{In this paper, we use the term $P_X$ to denote the (discrete) probability distribution of a (discrete) random variable $X$, i.e., the list of the probabilities of the outcomes, also known as the probability mass function. We use the term $F_X$ to denote the cumulative distribution function, i.e., $F_X(x)$ is the probability that $X$ will take a value less than or equal to $x$. More specifically, as the resource of interest is assumed to be completion time, we consider each leaf node, $v$, as a random variable, $X_v$, that has
a completion-time distribution $P_{X_v}$ and a cumulative distribution function (CDF) $F_{X_v}$.

For clarity, we elaborate regarding some common notations in this article. First, we denote all random variables by symbols of the form $X_{name}$ where in the subscript we put the name of the variable. %Similarly we denote nodes in the task trees by symbols of the form $v_{name}$ where the subscript contains the name of the node. 
Second, We denote the symbol $\tau_{name}$ where the subscript contains a name of a node corresponding to the sub-tree, i.e., the node is the root of the sub-tree. Note, the notation $X_{\tau_{v}}$ and $X_v$ may have the same meaning, a random variable represents the makespan of the sub-tree $\tau_{v}$ which is actually the duration distribution of node $v$.
Third, We use the symbol $X'_{name}$ to denote an approximation of the random variable $X_{name}$.}
...
is also NP-hard.
%Liat: fixed notation in the following pg
Stated in standard mathematical terms, leaf nodes represent primitive task durations as
independent discrete random variables \hl{$X_i$}. The duration of a sequence node $s$ is a random variable:
$$
...
Given a task tree $\tau$ as in figure ~\ref{fig:hierarchical plan}, we will compute the duration distribution in the form of \hl{PDF} for each compound task node including the root node, and then we will return the probability for $\tau$ to satisfy a given deadline $T$. 
...
\hl{In fact, as shown} in~\cite{mohring2001scheduling}, computing the CDF of a sum of random variables at a given point is NP-hard.
We thus define a notion of approximation, 
which we call an {\bf Kolmogorov upper bound} (defined below), and supply a $\Trim$ operator that reduces the support size, \hl{while} producing an approximation that is a Kolomogorov upper bound.
...
\]
Our notion of approximation uses the Kolmogorov distance. \hl{Let} $0\leq\varepsilon<1$. %$1 > \epsilon \geq 0$.
If the following equation holds:
\[
\forall ~ x, ~ F_X(x)+\varepsilon \geq F_{X'}(x) \geq F_X(x)
\]
%Liat: added \hl{approximation} 
...
the PDF of a random variable $X$ is represented by a list $D_X$, 
which consists of \hl{$d=(p', x)$} pairs, where $x\in \support(X)$
...
(in this case we denote the \hl{trim} operator
...
%Liat: all kinds of Trim...
\noindent We now show that with $D_X$ sorted in \hl{increasing order},
...
\begin{align*}
&F_{X'}(x) - F_{X}(x)  \\
 &{=} \sum_{i=0}^{l_x-1} (P_{X'}(x_i)- P_X(x_i\leq X <x_{i+1})) +(P_{X'}(x_{l_x})-P_X(x_{l_x} \leq X \leq x)) \\%[.25cm]
...
\hl{The makespan of a sequence} operator is a random variable $X$, the sum 
...
the support small, we apply Trim after each random variable is added.
\hl{Therefore, we compute the random variables 
...
computed as in Algorithm~\ref{alg:Trim}. That is, our approximation for a sequence operator (for $n\geq 2$) is given by: %Liat: add eq?
\begin{equation}
\label{eq:sequence}
Sequence(X'_1,\dots , X'_n,  \varepsilon) 
= Trim(Convolve(X'_1, Sequence(X'_2, \dots , X'_n, \varepsilon)), \varepsilon)
\end{equation}
% todo GERA: define Convolve
...
We denote the construction of \hl{the CDF using} Equation~\eqref{eq:par} by $\Parallel(X_1,\dots , X_n)$.
...
\end{proof}
%Liat: need to add some explanation for what is this lemma
...
\begin{algorithm}
 ...
 Let $v$ be the root of $\tau$  // Hence, $\tau_v=\tau$ \;
  ...
  \If{$v$  is a $\operatorname{Sequence}$ node}{
  	\For{$c \in \ch(v)$} {
	    \hl{$X'_c$} = \Network($\tau_c$, $\frac{|\tau_c|\varepsilon}{|\tau_v|}$)\;
	 }
	 \Return \Sequence(\hl{$\{X'_c\}_{c \in \ch(v)}$, $\frac{\varepsilon}{n_v|\tau_v|}$})	
...
We will present now a recursive approximation algorithm with a tighter bound on the error parameter for every sequence node. Specifically, we propose the generalized algorithm $\CalcNetwork$ (listed as Algorithm~\ref{alg:calc} below) whose main property is given using the algorithm $\EstimateError$ (listed as Algorithm~\ref{alg:BuildPolynomial} below) as follows:

%The algorithm $\EstimateError$, listed as Algorithm~\ref{alg:BuildPolynomial}, computes the approximation error for a tree $\tau$ given a function $\varepsilon$ that maps each of the network nodes in $\tau$ to the parameter passed in line 7 of the $\Network$ procedure, i.e., we use a generalized version of $\Network$, called $\CalcNetwork$ listed as Algorithm~\ref{alg:calc} below. 
...
\begin{theorem}\label{theorem:approxEps0}
	...
	distribution of the completion time of $\tau$; then, $\CalcNetwork(\tau, \varepsilon) \succeq_{\varepsilon_0} X_{\tau}$. 
\end{theorem}

\begin{proof}
By induction over the depth of $\tau$ denoted by $d$. Base: $ d=1$, a single primitive node \hl{$p$}. Primitive nodes do not carry any error, and by procedure \EstimateError line 3, \hl{$P_{X_p}=0$}, therefore  $\CalcNetwork(\tau_{p},\varepsilon) \succeq_0 X_{\tau_{p}}$. Induction hypothesis: Assume the lemma is true for a task tree with depth $ <d $. Step: we refer to the children nodes of the root node $r$, as $\ch(r)= \lbrace c_1, c_2,...,c_{|\ch(r)|}\rbrace$ 
	% TODO: move the definition of child way up
	as a set of sub task trees, each of them with depth smaller than $ d $. 
	If $r$ is a sequence node then,
	by Theorem~\ref{appSeqTheorem}, sequence node produce an error which is the sum of all children node errors and an additional $\varepsilon(r)$ caused by the trim operator $$\varepsilon_0={\varepsilon(r)+\sum_{c \in \ch(r)}{\EstimateError(\tau_c,\varepsilon)}}$$ equivalent to \EstimateError line 7. By the induction hypothesis, each of the children sub trees satisfy the lemma and we get that $\CalcNetwork(\tau_{r}, \varepsilon) \succeq_{\varepsilon_0} X_\tau$ as required.
%Liat: what about the epsilon?	
	\hl{If $r$ is a parallel node then,}
	by Lemma~\ref{tightPalTheorem}, $$\varepsilon_0=1-\prod_{c \in \ch(r)}{(1-\EstimateError(\tau_c))}$$ as in line 11 of \EstimateError. By the induction hypothesis, each of the children sub trees satisfy the lemma and we get that $\CalcNetwork(\tau_{r}, \varepsilon) \succeq_{\varepsilon_0} X_\tau$ as required.
\end{proof}

%Liat: Theorem 5 befor fix
%\begin{proof}
%By induction over the depth of $\tau$ denoted by $d$. Base: $ d=1$, a single primitive node $v_p$. Primitive nodes do not carry any error, and by procedure \EstimateError line 3, \hl{$P_{v_p}=0$}, therefore  $\CalcNetwork(\tau_{v_p},\varepsilon) \succeq_0 X_{\tau_{v_p}}$. Induction hypothesis: Assume the lemma is true for a task tree with depth $ <d $. Step: we refer to the children nodes of the root node $\ch(v_{root})= \lbrace c_1, c_2,...,c_{|\ch(v_{root})|}\rbrace$ 
%	% TODO: move the definition of child way up
%	as a set of sub task trees, each of them with depth smaller than $ d $. 
%	If $v_{root}$ is a sequence node then,
%	by Theorem~\ref{appSeqTheorem}, sequence node produce an error which is the sum of all children node errors and an additional $\varepsilon(v_{root})$ caused by the trim operator $$\varepsilon_0={\varepsilon(v_{root})+\sum_{c \in \ch(v_{root})}{\EstimateError(\tau_c,\varepsilon)}}$$ equivalent to \EstimateError line 7. By the induction hypothesis, each of the children sub trees satisfy the lemma and we get that $\CalcNetwork(\tau_{v_{root}}, \varepsilon) \succeq_{\varepsilon_0} X_\tau$ as required.
%%Liat: what about the epsilon?	
%	\hl{If $v_{root}$ is a parallel node then,}
%	by Lemma~\ref{tightPalTheorem}, $$\varepsilon_0=1-\prod_{c \in \ch(v_{root})}{(1-\EstimateError(\tau_c))}$$ as in line 11 of \EstimateError. By the induction hypothesis, each of the children sub trees satisfy the lemma and we get that $\CalcNetwork(\tau_{v_{root}}, \varepsilon) \succeq_{\varepsilon_0} X_\tau$ as required.
%\end{proof}



%Liat: need to add X'_c or it should be X_c?
\begin{algorithm}
  \DontPrintSemicolon
  \SetKwFunction{Sequence}{Sequence}
  \SetKwFunction{Parallel}{Parallel}
...
\begin{algorithm}
 ...
 Let $v$ be the root of $\tau$\;
  ...
  \If{$v$  is a $\operatorname{Sequence}$ node}{
	 ...
	 \Return \Sequence($\{X_c\}_{c \in \ch(v)}$, $\varepsilon(v)$)	
	...
	\If{$v$ is a $\operatorname{Parallel}$ node}{
		...
		\Return \Parallel($\{X_c\}_{c \in \ch(v)}$)
	}	
  }
%----------------------------------5
  
\caption{  \hl{$GenNetwork(\tau, \varepsilon)$}}
\label{alg:calc}
  
\end{algorithm}


Based on this theorem, we propose the following pseudo-algorithm for computing a tight approximation for the makespan of a task tree, as follows. Use some symbolic mathematical engine, such as Wolfram Mathematica (\url{www.wolfram.com/mathematica/}), to find a function $\varepsilon$ such that $\EstimateError(\tau,\varepsilon)$ is smaller than the approximation that you want to achieve; then, run $\CalcNetwork(\tau,\varepsilon)$ with this $\varepsilon$. This, of course, is only a pseudo algorithm because it is just a template, not dictating how to compute $\varepsilon$.
...
\end{lemma}
\begin{proof}
By induction over the depth of $\tau$ denoted by $d$. Base: if $d=1$ we have a tree with a single primitive node $v_p$. In this case  $\EstimateError(\tau,\varepsilon)$ which is indeed a polynomial degree zero.
Induction step: we refer to the children nodes of the root node, $\ch(v_{root})$, as a set of sub task trees, each of them with depth smaller than $d$. If $v_{root}$, is a sequence node 
$$\EstimateError(\tau,\varepsilon)=\sum_{c\in \ch(v_{root})}\EstimateError(\tau_c,\varepsilon) +  \varepsilon.$$ 
If $v_{root}$, is a parallel node  $$\EstimateError(\tau,\varepsilon)=1-\prod_{c\in \ch(v_{root})} (1-\EstimateError(\tau_c,\varepsilon)).$$ 
...
such that $F_{\CalcNetwork(\tau, \varepsilon)}(T) - F_{X_\tau}(T) = \EstimateError(\tau, \varepsilon)$ where $X_\tau$ is a random variable representing the completion time of $\tau$. 
\end{theorem}
\begin{proof}
...
The above result establishes that $\CalcNetwork$ is tight in the sense that there is no $\varepsilon_0 < \CalcNetwork(\tau, \varepsilon)$ 
such that $\CalcNetwork(\tau, \varepsilon)$ is always \hl{$\succeq_{\varepsilon_0}$} than $X_{\tau}$. 
...
random variable that is \hl{$\succeq_{\varepsilon}$} than $X_{\tau}$ with a smaller $\varepsilon$. 
...
%Liat: need mor refs to the ijcai paper
...
if \hl{$P(Y_i=y)=1/|\support(Y_i)|$} for every $i$ and every $y\in\support(Y_i)$, computing the probability $F_{\sum_{i=0}^{n} Y_{i}}(T)$ is equivalent, up to scaling by $\prod_{i=1}^n |\support(Y_i)|$, to counting the number of assignments to the random variables  such that $\sum_{i=0}^{n} Y_{i} \leq T$. 
...
the two-valued random variables  $Y_{1},\dots,Y_{n} $ with $P(Y_i=s_i)=1/2$ and $P(Y_i=0)=1/2$. 
By construction, there exists a subset of $S$ summing 
to $T$ if and only if $P(\sum_{i=0}^{n} Y_{i}=T)>0$.
...
that uses an \hl{inverted version of the} $\Trim$ operator, providing a {\em lower} bound of the CDF, as well as the upper bound
...
\begin{table*}[tbh!]
	{\footnotesize
		\begin{tabular}{|l|l|l|| p{1.5cm}|p{1.5cm}|p{1.8cm}||l|l|l|l|l|}
			\hline
			\multicolumn{1}{|c|}{\multirow{2}{*}{Task Tree}} & \multicolumn{1}{c|}{\multirow{2}{*}{$N$}} & \multicolumn{1}{c|}{\multirow{2}{*}{$M$}} & \multicolumn{3}{c|}{Approximation algorithm error, given $\varepsilon$} & \multicolumn{5}{c|}{Sampling algorithm error, given \# samples} \\ \cline{4-11} 
...
\begin{table*}[tbh!]
	{\footnotesize
		\begin{tabular}{|l|l|l|| p{1.5cm}|p{1.5cm}|p{1.5cm}||p{1.1cm}|p{1.1cm}|p{1.1cm}|}
			\hline
			\multicolumn{1}{|c|}{\multirow{2}{*}{Task Tree}} & \multicolumn{1}{c|}{\multirow{2}{*}{$N$}} & \multicolumn{1}{c|}{\multirow{2}{*}{$M$}} & \multicolumn{3}{c|}{Approximation algo. error, given $\varepsilon$} & \multicolumn{3}{c|}{Sampling algo. error, given \# samples} \\ \cline{4-9} 
...
\begin{table*}[htb!]
	{\footnotesize
		\begin{tabular}{|l|l|l|l||l|l|l||p{1cm}|p{1cm}|p{1cm}|}
			\hline
			\multicolumn{1}{|c|}{\multirow{2}{*}{Task Tree}} & \multicolumn{1}{c|}{\multirow{2}{*}{$N$}} & \multicolumn{1}{c|}{\multirow{2}{*}{$M$}} & \multirow{2}{*}{Exact} & \multicolumn{3}{c|}{Approx. algorithm, with $\varepsilon$} & \multicolumn{3}{c|}{Sampling algorithm, with \# samples} \\ \cline{5-10} 
			\multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & \multicolumn{1}{c|}{} &  & \multicolumn{1}{c|}{0.1} & \multicolumn{1}{c|}{0.01} & \multicolumn{1}{c|}{0.001} & \multicolumn{1}{c|}{$10^{3}$} & \multicolumn{1}{c|}{$10^{4}$} & \multicolumn{1}{c|}{$10^{5}$}  \\ \hline \hline
...
Now let:
\[
F_{X^-}(x) = \sum_{y\in D(Y)} P(Y=y) F_{X^-|Y=y}(x)
\]
and likewise:
\[
F_{X^+}(x) = \sum_{y\in D(Y)} P(Y=y) F_{X^+|Y=y}(x)
...
\begin{proof}
the runtime bound is obvious, as a trivial implementation simply encurs $m^{|Y|}$
...
%Liat: maybe add more info?
\hl{Trivial improvements} to the $\Trim$ operator are possible, 
such as not performing trimming (or even stopping a trimming operation) if the current support 
size is below $1/\varepsilon$, which may increase accuracy but also
the runtime. Another point is that in the combined algorithm, 
space and time complexity can be reduced by adding some $\Trim$ operations,
...
``distance'' alnog the $x$ parameter over which it is moved. The latter may be arbitrarily bad for estimating the expectation. Nevertheless, a different version of \hl{$Trim$} that bounds just this distance was
shown to provide a polynomial-time approximation scheme
for the expectations in EXPECTI-MIN-MAX game trees
\cite{shperbergmonte}, 
if the utilities are bounded. Since the expectation
operator involves convolution, these results should be
applicable (with some adjustment) to task networks as well.
