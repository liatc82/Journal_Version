...
\begin{abstract}
	% TODO: Extend this abstract to include also Eyal's part and the tightning analysis.
	...
	where our scheme is superior to sampling and to exact computation using some task networks. \hl{Tightening the analysis, P-trees,  } 
\end{abstract}  
...
% TODO: Eyal, can you please add your contributions to the above paragraph?
...
Primitive action nodes contain distributions over their resource consumption. Many resources can be modeled using the proposed approach. 
%For example, if the resource of interest is fuel one can consider parallel execution as bus or carpool and sequential execution as using a private car. 
For example if the resource of interest is memory, tasks running in parallel use the sum of the memory space of each of the tasks; if they run in sequence, only the maximum thereof is needed. Note that the role parallel and sequence node is inversed here. We will assume henceforth, in order to be more concrete, that the resource of interest is time, i.e., that parallel nodes represent maximum and sequence nodes represent sum.

%(for example, if fuel is the given resource, one can consider parallel execution as bus or carpool and sequential execution as driving a private car e.g., memory: tasks running in parallel use the sum of the memory space of each of the tasks; if they run in sequence, only the maximum thereof is needed)
...
For clarity, we use the following standard for notations in the paper. First, we denote all random variables by symbols of the form $X_{name}$ where in the subscript we put the name of the variable. %Similarly we denote nodes in the task trees by symbols of the form $v_{name}$ where the subscript contains the name of the node. 
...
%Computing the probability that $\tau $ will satisfy the deadline $T$ (i.e. terminate in time $t \leq T$) is called here the ``deadline problem".
%\end{definition}
...
%\hl{We denote by $\ch(v)$ the set of children of some node $v$.}
...
%and denote the cumulative distribution function (CDF) of a random variable $X$ by $F_X$. 
...
% TODO *** Why do we need to sort? If the input is sorted, and we keep it sorted? ***
...
% therefor $\GenNetwork(\tau_{p},\varepsilon) \succeq_0 X_{\tau_{p}}$
%Primitive nodes do not carry any error and, by line 3 of \EstimateError, $\EstimateError(\tau_p,\varepsilon)=0$, therefore  $\GenNetwork(\tau_{p},\varepsilon) \succeq_0 X_{\tau_{p}}$. 
...
%we refer to the children nodes of the root node $r$, as $\ch(r)= \lbrace c_1, c_2,...,c_{|\ch(r)|}\rbrace$ as a set of sub task trees, each of them with depth smaller than $d$. 
...
%todo: need more refs from the ijcai paper
...
\paragraph{\bf Focused trimming} Another option is, when given a specific threshold, trying for higher accuracy in just the region of the threshold, but how to do that is non-trivial. For {\em sampling} schemes such methods are known, including adaptive sampling~\cite{bucher1988adaptive,lipton1990praccctical}, stratified sampling, and other schemes. It may be possible to apply such schemes to deterministic algorithms as well - an interesting issue for future work. 
...
\paragraph{\bf Optimal trimming} While we proved that the trimming procedure proposed in this paper allows for approximation that improves polynomially with the time and memory invested. It is interesting to look for optimal approximations. In~\cite{cohen2018}, we showed that an optimal approximation of a single random variable can be obtained in polynomial time. \hl{Given a random variable $X$ and a target support size $m$, a new random variable $X'$ is generated such that $\forall t, F_{X'}(t)-F_X(t)$ in minimal}. An algorithm for optimal approximation the makespan of a complete plan is still missing.

\paragraph{\bf Compact representations of the random variables} One can view the work presented in this paper in the context of function approximation. In general, a function approximation problem is about the selection a function among a well-defined class that approximates a target function in a certain way. In our case, we approximate the CDF of a random variable with a piecewise constant function with a small number of pieces. As in other applications of function approximation, it is natural to ask whether more compact representations of the random variable exist. For example one can represent functions in a compressed from where a repeated entry can be specified once with a number that specifies the number of repetitions. Another approach would be to approximate using, e.g., splines instead of constant lines. The challenge will be, in any of these variants, to work directly on the compressed representation, as we do in this paper.

\paragraph{\bf Split weights} In the proposed $\Trim$ algorithm, the inner loop goes until $p+prob(d) \leq \varepsilon$ and, when this condition is not met, the value of $prod(d)$ is left for the next iteration. A possible improvement, not included in the base version for simplicity, is to add to $p$ the part of $prob(d)$ up to $\varepsilon$ (i.e., have $p=\varepsilon$) and leave only the remaining part of $prob(d)$ to the next iteration.
...
This chapter surveys previous work on HTN planning, series-parallel networks, scheduling with uncertain task durations, the sum and the maximum of random variables, and approximation schemes.

HTN (Hierarchical Task Network) planning is an approach to automated planning where tasks are connected together by the use of task networks, which are collections of tasks and constraints on those tasks. There are three types of tasks in this model: ``primitive" tasks, ``goal" tasks and ``compound" tasks~\footnote{Also called "high-level" actions or ``non-primitive" tasks.}. Task networks also include constraints on the tasks, which may restrict how some of the variables can be bound, and the order in which the tasks are to be performed, \cite{erol1994htn,erol1996complexity, Russell:2003:AIM:773294}. In~\cite{erol1994htn} Erol et al., formally define, analyze and explicate features of the design of HTN planning systems. Specifically, how is the complexity of HTN planning varies with various conditions on the task networks. Methods for solving HTN are suggested as an online planning~\cite{nau1998control, nau2003shop2, gabaldon2002programming} and as offline planning~\cite{kelly2008offline}. In the experiments we conducted, we used hierarchical plans obtained by SHOP2~\cite{nau2003shop2} planner in the ``Logistics" domain from IPC2 (\href{url}{http://ipc.icaps-conference.org/}). The SHOP2, Simple Hierarchical Ordered Planner 2, is a domain-independent planning system based on Hierarchical Task Network (HTN) planning. 
There are other HTN planners like TLPLan or TALPlanner ~\cite{kvarnstrom2000talplanner} but we chose for convenience reasons to use JSHOP2, the Java version of SHOP2.

There has been much work on series-parallel networks, although not all related to planning or AI. In ~\cite{gelenbe1989multiprocessor}, the author discusses the fundamental issues involved in the performance of parallel computers. In Chapter 5 of Gelenbe's paper, a model for series–parallel processing structures is presented. Programs in this model are composed of (primitive) tasks; some of them are to be performed in series, others may be performed in parallel. Given the execution time distribution of each task (i.i.d) and the characteristic parameters of the branching process, a method to compute numerically the execution time distribution of the program is shown. The computation involves numerical solutions based on solving a differential-integral equation and iterative methods. In~\cite{gutjahr1992average}, which is based on the same model, a bound on the average total execution time of a series–parallel processing structure is presented. Both papers are very relevant to our work and contributed as case studies. Moreover, this type of work encouraged us to examine the deadline problem and provide guarantees for the probability of meeting a deadline. This type of analysis is needed in Service-Level-Agreements (SLA) where guarantees of the form: ``response time less than 1mSec in at least 95\% of the cases'' are common~\cite{buyya2011sla}.


Scheduling and in particular, scheduling under uncertainty, can provide additional motivation to our work. In \cite{herroelen2005project} they review the fundamental approaches for scheduling under uncertainty as reactive scheduling, stochastic project scheduling etc. and discuss the potentials of these approaches for scheduling under uncertainty projects with deterministic network evolution structure. This paper provides a good survey of scheduling under uncertainty. An important paper is ~\cite{hagstrom1988computational} which provides computational complexity results for two PERT problems. Here a project is specified by precedence relations among tasks which is given as a graph and the task durations are independent random variables with discrete, finite ranges. Three results are obtained: computing a value of the cumulative distribution function of project duration is $\#P$-complete, computing the mean of the distribution is at least as hard, and neither of the problems can be computed in time polynomial in the number of points in the range of the project duration. This paper deals with a more general problem than ours, and with a different type of complexity. Our results are orthogonal, because we show a source of complexity that is not in the graph structure but in the distributions themselves. In fact, the 2-state problem shown to be hard for general graphs by Hagstrom, can be solved in polynomial time for series-parallel trees by dynamic programming. \hl{In} \cite{cohen2015estimating}.
Another relevant paper is \cite{beck2007proactive} which allows to represent each activity by an independent random variable with a known mean and variance. The best solutions are ones which have a high probability of achieving a good makespan, and methods how Monte Carlo simulation can be
combined with deterministic scheduling algorithms to solve this problem are shown. Furthermore, their bounds give a probability of error while we assure that large errors can not happen. 
In \cite{fu2010towards} RCPSP/max (Resource Constrained Project Scheduling Problems
with minimum and maximum time lags) is studied, where the durations of activities are not known with certainty. In \cite{bonfietti2014disregarding} a simulation approach is used to evaluate the expected makespan of a number of Partial Order
Schedules, obtained by solving
scheduling benchmarks via multiple approaches. 
The evaluation in the paper shows correlation between the expected makespan and the makespan obtained by simply fixing all durations to their average. The strength of the correlation is such that it is possible to disregard completely the uncertainty during the schedule construction and yet obtain a very accurate estimation of the expected makespan.
...
Various works exist in the research literature regarding the sum and the maximum of random variables. \cite{evans2004algorithms} present algorithms for computing the exact probability density function of the sum of two independent discrete random variables, and show an implementation of the algorithm in a computer algebra system. This paper do not examine the case of summing more than two discrete random variables which is our main challenge. 
However, there are other papers that handle the case of summing more than two random variables like  \cite{hong2013computing} which examines the sum of Bernoulli distributed random variables. They present a simple derivation for an exact formula with a closed-form expression
for the CDF of the Poisson binomial distribution. The derivation uses the discrete Fourier
transform of the characteristic function of the distribution. Numerical studies were conducted to study the accuracy of the developed algorithm and approximation methods.
\cite{bromiley2013products}, examines the sum of normally distributed random variables. The fact that the product and the convolution of Gaussian probability density functions (PDFs) are also Gaussian functions, is well known. This paper provides proofs that the following cases are also Gaussian functions: the product of two univariate Gaussian PDFs, the product of an arbitrary number of univariate Gaussian PDFs, the product of an arbitrary number of multivariate Gaussian PDFs, and the convolution of two univariate Gaussian PDFs.
In \cite{mercier2007discrete} the analysis is limited to i.i.d. random variables, they propose algorithms to compute bounds for cumulative density functions of sums of i.i.d. non-negative random variables, renewal functions and cumulative density functions of geometric sums of i.i.d. non-negative random variables. Distribution of the maximum of random variables was studied in~\cite{devroye1980generating}, with a focus mostly on continuous distributions. The above papers treat either the sum of random variables or the maximum of random variables but not both together. 

The fact that the deadline problem is NP-hard, led us to derive an efficient approximation algorithm and to explore the area of approximation algorithms. We examined a well known approximation algorithms for the $\operatorname{KNAPSACK}$ problem~\cite{lai2006knapsack, lawler1979fast}, and tried to manipulate it to fit as an approximation algorithm for the deadline problem. Recall the $\operatorname{KNAPSACK}$ problem, given a knapsack of size $B \in Z^+$ and a set $S = \{a_1, \dots , a_n\}$ of objects with corresponding sizes and profits $s(a_i) \in Z^+$ and $p(a_i)\in Z^+$, what is the optimal subset of objects whose total size is bounded by $B$ and has the maximum possible total profit? The idea of the FPTAS for the $\operatorname{KNAPSACK}$ problem is to scale the profits down enough so the profits of all the objects are polynomially bounded in $n$, then, use dynamic programming on the new instance, and return the resulting subset. By scaling with respect to a desired $\varepsilon$, the solution is at least $(1-\varepsilon)\cdot OPT$ where $OPT$ is the optimal solution, in polynomial time with respect to both $n$ and $1/\varepsilon$. Reading this paper was the first step towards our approximation algorithm and the understanding that simple binning will not do.
An approximation algorithms for a variant of the \textit{SubsetSum} problem \cite{Cormen:2001:IA:580470} uses the same operator as our $\Trim$, but in different context. This \textit{SubsetSum} problem is a decision problem, given a set of $n$ numbers $x_1, \cdots x_n$, a target $t$ and $0\leq\varepsilon\leq 1$, return
yes, if there is a subset that sums between $(1 - \varepsilon)t$ and $t$. In this FPTAS, 
``trimming" is to remove values that are close to each other. In other words, if two values $s_1$ and $s_2$ which represent a sum of subset of $S$ are close to each other, then for the purpose of finding an approximate solution there is no reason to maintain both of them, and it is possible to merge them to be represented by $s_1$ or $s_2$ and delete the other. Clearly, the idea in our approximation algorithm is similar to this idea. Referring to the deadline problem as a counting problem motivated us to seek for other type of approximation algorithms. In \cite{arora2009computational}, Chapter 9  ``complexity of counting", the complexity class $\#$P is studied and examples for counting problems are given, e.g., $\#\operatorname{KNAPSACK}$ problem (defined in Chapter \ref{sec:complexity}). The following two papers provide a randomized polynomial-time approximation schemes (FPRAS) to the $\#\operatorname{KNAPSACK}$ problem.
The first, \cite{dyer2003approximate}, presents an algorithm to sample uniformly, and count approximately the solutions to a the $\#\operatorname{KNAPSACK}$ problem. The algorithm is using dynamic programming to provide a deterministic relative approximation and then uses sampling techniques to give arbitrary approximation ratios. The second, \cite{morris2004random}, suggests different approach and uses Markov chain Monte Carlo technique.
A fully polynomial-time approximation scheme (FPTAS) to solve the $\#\operatorname{KNAPSACK}$ problem is shown in \cite{gopalan2011fptas}, this algorithm is also based on dynamic programming.
Another type of approximation uses Monte-Carlo technique are presented in ~\cite{bucher1988adaptive,lipton1990practical}. We suggested an additive and deterministic approximation algorithm, as future work can be interesting to consider a randomized polynomial-time approximation schemes.
