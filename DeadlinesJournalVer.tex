...
%\end{definition}
\hl{In words, given a task tree $\tau $ and a deadline $T$, what is the probability that $\tau $ 
...
is also NP-hard.
% TODO: move the definition of child here
% TODO: Say that we are now describing the random variable that assigned to each node in the tree.
...
The value $P_{\text{\hl{X}}}(x_i < X < x_{i+1})$ equals the value of $p$ in
...
In this case, by line 3 of Algorithm~\ref{alg:calc}, $\CalcNetwork(\tau_p,\varepsilon)=X_p$ and, by line 3 of Algorithm~\ref{alg:BuildPolynomial},  $\EstimateError(\tau_p,\varepsilon)=0$ and the claim follows.
% therefor $\CalcNetwork(\tau_{p},\varepsilon) \succeq_0 X_{\tau_{p}}$
%Primitive nodes do not carry any error and, by line 3 of \EstimateError, $\EstimateError(\tau_p,\varepsilon)=0$, therefore  $\CalcNetwork(\tau_{p},\varepsilon) \succeq_0 X_{\tau_{p}}$. 
...
If $r$ is a sequence node then,	by Theorem~\ref{appSeqTheorem}, sequence nodes produce an error which is the sum of all child nodes errors and an additional $\varepsilon(r)$ caused by the trim operator $$\varepsilon_0={\varepsilon(r)+\sum_{c \in \ch(r)}{\EstimateError(\tau_c,\varepsilon)}}$$ equivalent to \EstimateError line 7. By the induction hypothesis, each of the children sub trees satisfy the lemma and we get that $\CalcNetwork(\tau_{r}, \varepsilon) \succeq_{\varepsilon_0} X_\tau$ as required.
	If $r$ is a parallel node then,
	by Lemma~\ref{tightPalTheorem}, $$\varepsilon_0=1-\prod_{c \in \ch(r)}{(1-\EstimateError(\tau_c,\varepsilon))}$$ as in line 11 of \EstimateError. By the induction hypothesis, each of the child subtrees satisfy the lemma and we get that $\CalcNetwork(\tau_{r}, \varepsilon) \succeq_{\varepsilon_0} X_\tau$ as required.
\end{proof}
